...
open_status_dynamic_price = dict(
    getOrders= dict(
        dynamicPriceExpiry=1612305372
    )
)

completed_status_dynamic_price = dict(
    getOrders= dict(
        dynamicPriceExpiry=1612304472
    )
)
...
def test_coins_price(p2p_user):
    ...
    all_coins_price = p2p_user.getPrices()
...
def test_coin_price(p2p_user):
    ...
    current_bitcoin_price = p2p_user.getCurrentPrice(orderSide="buy", currency="bitcoin")
...
def test_open_dynamic_price(p2p_user):
    ...
    open_dynamic_price = p2p_user.getDynamicPriceExpiry()
...
def test_completed_dynamic_price(p2p_user):
    ...
    completed_dynamic_price = p2p_user.getDynamicPriceExpiry(status="completed")
...
def test_place_limit_order(p2p_user):
    ...
    bitcoin_limit_order = p2p_user.placeLimitOrder("buy", 1, "bitcoin", 16000000)
...
def test_post_market_order(p2p_user):
    ...
    bitcoin_market_order = p2p_user.postMarketOrder("sell", 0.01, "bitcoin")
...
def test_get_orders(p2p_user):
    ...
    open_orders = p2p_user.getOrders("open")
...
def test_get_market_book(p2p_user):
    ...
    market_history = p2p_user.getMarketBook()
